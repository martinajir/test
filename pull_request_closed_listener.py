#!/usr/bin/env python3
"""Receive GitHub webhook deliveries for closed pull requests."""
...
LOGGER = logging.getLogger("pull_request_closed_listener")
...
def handle_webhook(
    ...
    try:
        ...
        return WebhookResponse(
            400,
            "rejected",
            "Closed pull request payload is missing required fields",
        )
    ...
    if any(type(value) is not expected for value, expected in fields):
        return WebhookResponse(
            400,
            "rejected",
            "Closed pull request payload has invalid field types",
        )

    outcome = "merged" if merged else "closed without merging"
    message = (
        f"Pull request #{number} in {repository} was {outcome} "
        f"by {sender}: {title}"
    )
